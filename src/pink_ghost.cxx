...
PinkGhost::PinkGhost(Position pos)
        : eaten_(false),
          weak_(false),
          radius_(grid_size/2),
          pos_(pos),
...
PinkGhost
PinkGhost::velocity_next(Walls w, Position redPos, Position dest)
{
    // updating the velocity

    // skip if it is eaten
    if(eaten_){
        PinkGhost result(*this);
        return result;
    }
    ...
    if(pos_.x % grid_size == 0 && pos_.y % grid_size == 0) {
        ...
        if(weak_) {
            ...
            Position redToPlayer = {dest.x - redPos.x, dest.y - redPos.y};
            dest.x = dest.x + redToPlayer.x;
            dest.y = dest.y + redToPlayer.y;

            // if this is not a wall or off the board path find
...
bool PinkGhost::hits(Player player, int& score)
{
